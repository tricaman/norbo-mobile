...
import { DEFAULT_KINDS } from "@/components/tools/places/kind-meta";
...
/**
 * DogFriendlyPlacesTool — full-screen map of dog parks, dog-friendly venues,
 * beaches, green areas, trails and pet-care infrastructure, from our own
 * OSM-imported dataset.
 *
 * The persisted tool input is the LAYER SELECTION ONLY — never coordinates
 * (persisted inputs sync to the server; we do not store user locations).
 */
const DogFriendlyPlacesTool: ToolComponent<"dog-friendly-places"> = ({
  ...
  const [kinds, setKinds] = React.useState<PlaceKind[]>(
    initialInputs?.kinds ?? DEFAULT_KINDS,
  );
  ...
  React.useEffect(() => {
    const payload: Inputs = { kinds: debouncedKinds };
  ...
  async function onNearMePress() {
    if (!location.granted && !location.canAskAgain) {
      ...
      toast.show({
        type: "warning",
        title: t("tools.dogFriendlyPlaces.locationDenied"),
      });
    ...
    Alert.alert(
      t("tools.dogFriendlyPlaces.nearMe"),
      t("tools.dogFriendlyPlaces.locationRationale"),
      [
  ...
  return (
    <View style={styles.root}>
      ...
      <PlaceKindFilterBar value={kinds} onChange={setKinds} />
      ...
      {(tooWide || data?.truncated) && (
        <View style={styles.hint} pointerEvents="none">
          <Text style={styles.hintText}>
            {t("tools.dogFriendlyPlaces.zoomIn")}
          </Text>
        </View>
      )}
